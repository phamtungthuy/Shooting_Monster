...
bool MainObject::LoadImg(std::string path, SDL_Renderer* screen){
    ...
    if(ret == true){
        width_frame_ = rect_.w / 8; // so frame
...
void MainObject::HandelInputAction(SDL_Event events, SDL_Renderer* screen){
    if(events.type == SDL_KEYDOWN){
        ...
        else if(events.key.keysym.sym == SDLK_p) x_pos_ = 0;
    }
    ...
    if(events.type == SDL_MOUSEBUTTONDOWN){
        if(events.button.button == SDL_BUTTON_RIGHT){
            ...
            if(status_ == WALK_LEFT){
                ...
                p_bullet->SetRect(this->rect_.x + 10 , rect_.y + height_frame_* 0.25);
            }
            else{
...
void MainObject::CheckToMap(Map& map_data){
    ...
    */
    ...
    x2 = (x_pos_ + x_val_ + width_frame_ - 1)/TILE_SIZE; // tru di mot luong sai so de check va cham

    y1 = (y_pos_ )/TILE_SIZE;
    y2 = (y_pos_ + height_min - 1)/TILE_SIZE; // tru di mot luong sai so de check va cham

    if(x1 >= 0 && x2 < MAX_MAP_X && y1 >=0 && y2 < MAX_MAP_Y){
         // di chuyen ve ben phai
        if(map_data.tile[y1][x2] == DESTINATION || map_data.tile[y2][x2] == DESTINATION || map_data.tile[y1][x1] == DESTINATION || map_data.tile[y2][x1] == DESTINATION) come_to_destination = true;
        if(x_val_ > 0){
